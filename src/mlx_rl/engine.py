...
def prefill_cache(model, prompt_ids: list[int], prefill_step_size: int = 2048,
    ...
    'prompt' whose forward produces the first completion logits."""
...
def _make_generator(model, stops, max_new_tokens, temperature, completion_batch_size,
                    group_size, prefill_batch_size, prefill_step_size):
    return _RolloutBatchGenerator(
        model,
...
def _insert_group(gen, model, prompt, group_size, share_prompt,
                  prefill_step_size) -> list[int]:

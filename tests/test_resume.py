...
def test_resumed_run_matches_uninterrupted_run(tmp_path):
    ...
    def cmd(steps, out, *extra):
        # --no-manage-machine: this is the only test that spawns the trainer,
        # so it is the only one that would take the machine-wide exclusive
        # memory lease. A 0.5B / 48-token run needs no arbitration, and
        # taking the lease makes the test fail on any box where another job
        # holds it (MachineBusyError) — i.e. flaky by construction on a
        # machine that is deliberately kept busy.
        return [sys.executable, "-m", "mlx_rl.train", "--task", "arithmetic",
                ...
                "--eval-n", "4", "--seed", "0", "--no-manage-machine",
                "--out", str(out), *extra]

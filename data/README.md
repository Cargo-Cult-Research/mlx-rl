...
categories) used by the `honesty` task's `papers` domain as its item pool and
search index, so training never hits the live arXiv API. Built by
`scripts/fetch_arxiv_snapshot.py`, captured 2026-08-16: 97 hand-listed
well-known papers, the first 40 cs.LG submissions of every month 2023-01..2026-08, and 300 `fictional_*` titles —
